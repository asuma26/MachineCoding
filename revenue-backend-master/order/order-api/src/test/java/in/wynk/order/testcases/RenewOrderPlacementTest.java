...
public class RenewOrderPlacementTest {
    ...
    @SneakyThrows
    public void setup() {
        Mockito.when(orderDao.save(Mockito.any())).thenReturn(OrderMockData.buildRenewPersistableFulfilledOrder());
        Mockito.when(orderDao.findById(Mockito.eq(OrderMockConstants.MOCK_ORDER_ID))).thenReturn(Optional.ofNullable(OrderMockData.buildRenewPersistableAcknowledgedOrder()));
        Mockito.when(restTemplate.postForObject(Mockito.eq(subscribePlanEndPoint), Mockito.any(), Mockito.eq(PlanProvisioningResponse.class))).thenReturn(OrderMockData.buildSubscriptionProvisioningResponse());
    }

    @Test
    @Order(1)
    public void run() {
        OrderResponse orderResponse = orderManager.placeOrder(OrderMockConstants.DUMMY_PARTNER, OrderMockData.buildRenewOrderPlacementRequest());
        ...
        Assert.assertEquals(((OrderNotificationResponse) orderResponse).getOrderDetails().getType(), PaymentEvent.SUBSCRIBE);

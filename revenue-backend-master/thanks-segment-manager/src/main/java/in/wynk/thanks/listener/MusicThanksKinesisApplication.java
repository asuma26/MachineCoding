...
@Profile("!local")
@Service
public class MusicThanksKinesisApplication {

    private static final Logger logger = LoggerFactory.getLogger(MusicThanksKinesisApplication.class);

    private static Executor executor = Executors.newFixedThreadPool(4);
    @Value("${music.thanks.kinesis.stream}")
    private String applicationStreamName;
    @Value("${music.thanks.kinesis.app}")
    ...
    public void consume() throws UnknownHostException {
        ...
        kinesisClientLibConfiguration.withInitialPositionInStream(APPLICATION_INITIAL_POSITION_IN_STREAM).withMetricsLevel(NONE);

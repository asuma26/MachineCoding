...
public class PaymentRenewalChargingSQSMessageExtractor extends AbstractSQSMessageExtractor {
    ...
    @Value("${payment.pooling.queue.charging.sqs.messages.extractor.waitTimeInSeconds}")
    private int waitTimeInSeconds;

    private final String queueName;


    public PaymentRenewalChargingSQSMessageExtractor(String queueName,   @Qualifier(BeanConstant.SQS_MANAGER) AmazonSQS sqs) {
        super(sqs);
